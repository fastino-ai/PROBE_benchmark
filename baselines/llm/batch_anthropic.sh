...
if [ -z "$ANTHROPIC_API_KEY" ]; then
    echo "Error: ANTHROPIC_API_KEY is not set. Please set it in .env file or as an environment variable."
    exit 1
fi

cd ~/Documents/proactive-baselines

echo "🎯 Running Batch Inference with Anthropic"
echo "======================================================="
echo ""
python run_native_batch_evaluation.py \
    --models \
        MODELS.ANTHROPIC.CLAUDE_4_OPUS \
